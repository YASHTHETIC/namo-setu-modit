  ...
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5">
        <MetricTile label="Temples" value={stats.temples} icon={MapPin} />
        <MetricTile label="Darshan" value={stats.darshan_bookings.toLocaleString()} icon={Calendar} delta={12} deltaLabel="%" />
        <MetricTile label="Puja" value={stats.puja_bookings.toLocaleString()} icon={TrendingUp} />
        <MetricTile label="Donations" value={`₹${(stats.donation_amount / 100000).toFixed(1)}L`} icon={Heart} delta={8} deltaLabel="%" />
        <MetricTile label="Stays" value={stats.accommodation_bookings} icon={Users} />
        ...
        <div className="p-8">
          ...
          {festivals.data?.length ? (
            <div className="space-y-4">
              {festivals.data.slice(0, 5).map((festival) => (
                <div
