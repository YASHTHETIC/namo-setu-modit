              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-9 w-full rounded-lg border border-[var(--border)] bg-white pl-9 pr-3 text-xs focus:outline-none focus:ring-1 focus:ring-[var(--brand)]"
                  />
                </div>
              </div>
                ...
                <div className="space-y-1.5">
                  <button
                    ...
                    className={`block w-full rounded-lg px-3 py-1.5 text-left text-xs transition-colors ${
                      !selectedCategory ? "bg-[var(--brand)]/10 font-semibold text-[var(--brand)]" : "text-[var(--text-secondary)] hover:bg-[var(--bg-subtle)]"
                    }`}
                  >
                    All Categories
                  </button>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="h-8 w-full rounded-lg border border-[var(--border)] bg-white px-2 text-xs focus:outline-none focus:ring-1 focus:ring-[var(--brand)]"
                    placeholder="Min"
                  />
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {[
                    ...
                    <button
                      ...
                      className="rounded-full border border-[var(--border)] px-2 py-0.5 text-[10px] text-[var(--text-muted)] hover:border-[var(--brand)] hover:text-[var(--brand)] transition-colors"
                    >
                      {preset.label}
                    </button>
                  ))}
                  ...
                  {[4, 3, 2, 1].map((r) => (
                    <button
                      ...
                      className={`flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs transition-colors ${
                        minRating === r ? "bg-[var(--brand)]/10 text-[var(--brand)]" : "text-[var(--text-secondary)] hover:bg-[var(--bg-subtle)]"
                      }`}
                    >
                      <div className="flex">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star key={s} className={`h-3 w-3 ${s <= r ? "fill-[var(--brand)] text-[var(--brand)]" : "fill-gray-200 text-gray-200"}`} />
                        ))}
                      </div>
                      <span>& Up</span>
                    </button>
        ...
        <div className="flex-1">
          ...
          <div className="mb-4 flex items-center justify-between rounded-xl border border-[var(--border)] bg-white px-4 py-3">
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-1.5 rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs font-medium text-[var(--text-secondary)] hover:border-[var(--brand)] hover:text-[var(--brand)] transition-colors lg:hidden"
              >
            ...
            <div className="flex items-center gap-3">
              <select
                ...
                className="rounded-lg border border-[var(--border)] bg-white px-3 py-1.5 text-xs text-[var(--text-secondary)] focus:outline-none focus:ring-1 focus:ring-[var(--brand)]"
              >
              ...
              <div className="hidden items-center rounded-lg border border-[var(--border)] sm:flex">
                <button
                  ...
                  className={`p-1.5 ${viewMode === "grid" ? "bg-[var(--brand)]/10 text-[var(--brand)]" : "text-[var(--text-muted)]"}`}
                >
                  <Grid3X3 className="h-4 w-4" />
                </button>
                <button
                  onClick={() => setViewMode("list")}
                  className={`p-1.5 ${viewMode === "list" ? "bg-[var(--brand)]/10 text-[var(--brand)]" : "text-[var(--text-muted)]"}`}
                >
          ...
          {hasActiveFilters && (
            <div className="mb-4 flex flex-wrap gap-2">
              {selectedCategory && (
                <span className="inline-flex items-center gap-1 rounded-full bg-[var(--brand)]/10 px-3 py-1 text-xs font-medium text-[var(--brand)]">
                  {categories.find((c) => c.slug === selectedCategory)?.name}
                  <button onClick={() => setSelectedCategory("")}><X className="h-3 w-3" /></button>
                </span>
              )}
              {selectedBrands.map((b) => (
                <span key={b} className="inline-flex items-center gap-1 rounded-full bg-[var(--brand)]/10 px-3 py-1 text-xs font-medium text-[var(--brand)]">
                  {b}
      ...
      <div className="group rounded-xl border border-[var(--border)] bg-white transition-all hover:-translate-y-0.5 hover:shadow-lg overflow-hidden">
        ...
        <div className="relative h-48 overflow-hidden bg-gradient-to-br from-orange-50 via-amber-50 to-orange-100">
          ...
          {product.seller.isVerified && (
            <span className="absolute top-2 right-2 rounded-lg bg-emerald-500 px-2 py-0.5 text-[10px] font-bold text-white">
      ...
      <div className="group flex gap-4 rounded-xl border border-[var(--border)] bg-white p-4 transition-all hover:-translate-y-0.5 hover:shadow-lg">
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-1 rounded bg-[var(--brand)]/10 px-1.5 py-0.5">
              <span className="text-xs font-bold text-[var(--brand)]">{product.rating}</span>
              <Star className="h-2.5 w-2.5 fill-[var(--brand)] text-[var(--brand)]" />
            </div>
          </div>
            ...
            <div className="ml-auto">
              <button
                ...
                className={`flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-semibold transition-all ${
                  added ? "bg-emerald-500 text-white" : "bg-[var(--brand)] text-white hover:bg-[var(--brand-dark)]"
                }`}
              >
...
export default function ProductsPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center py-20"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[var(--border)] border-t-[var(--brand)]" /></div>}>
      <ProductsContent />
    </Suspense>
  );
}
