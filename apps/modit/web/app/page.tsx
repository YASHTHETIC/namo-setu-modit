...
const CATEGORIES = [
  { name: "Cement", icon: "ðŸ—ï¸", slug: "cement", count: "500+ Products" },
  { name: "Steel & TMT", icon: "ðŸ”©", slug: "steel", count: "300+ Products" },
  { name: "Tiles", icon: "ðŸ”²", slug: "tiles", count: "1200+ Products" },
  { name: "Paint", icon: "ðŸŽ¨", slug: "paint", count: "800+ Products" },
  { name: "Electrical", icon: "âš¡", slug: "electrical", count: "600+ Products" },
  { name: "Plumbing", icon: "ðŸ”§", slug: "plumbing", count: "400+ Products" },
  { name: "Sand & Aggregate", icon: "ðŸª¨", slug: "sand", count: "150+ Products" },
  { name: "Bricks & Blocks", icon: "ðŸ§±", slug: "bricks", count: "200+ Products" },
  { name: "Wood & Plywood", icon: "ðŸªµ", slug: "wood", count: "350+ Products" },
  { name: "Hardware", icon: "ðŸ”©", slug: "hardware", count: "1000+ Products" },
  { name: "Bathroom", icon: "ðŸš¿", slug: "bathroom", count: "250+ Products" },
  { name: "Doors & Windows", icon: "ðŸšª", slug: "doors", count: "180+ Products" },
];
...
const HERO_SLIDES = [
  ...
  { badge: "NEW ARRIVAL", badgeColor: "#00875A", title: "Premium Tiles Collection", sub: "Explore 500+ designs from Kajaria, Somany, Johnson. Starting â‚¹42/sqft.", cta: "Explore", ctaStyle: "green", link: "/products?category=tiles" },
];
    ...
    <div className="product-card">
      ...
      <div className="product-info">
        ...
        <div className="product-price">
          <span className="current">â‚¹{p.price.toLocaleString()}</span>
          <span className="mrp">â‚¹{p.mrp.toLocaleString()}</span>
        ...
        <p className="product-delivery">
          ...
          <span className="free">Free delivery</span> Â· {p.delivery.replace("Delivery ", "")}
        </p>
        ...
        <div className="flex gap-2 mt-3">
          <button className="btn-amber flex-1 text-[11px] py-1.5 px-3">Add to Cart</button>
...
export default function ModitHomePage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      <div className="bg-gradient-to-r from-[var(--brand)] via-[var(--brand-hover)] to-[var(--brand)] text-white text-center py-1.5 px-4 text-[11px] font-semibold">
        <span className="hidden sm:inline">Free delivery on first order </span>
        <span className="mx-2 text-white/30">|</span>
        <span>MONSOON MEGA SALE â€” Up to 25% OFF</span>
        <span className="mx-2 text-white/30">|</span>
        ...
        <div className="max-w-[1440px] mx-auto flex h-[60px] items-center gap-3 px-4 sm:px-6">
          ...
          <Link href="/" className="flex shrink-0 items-center gap-1.5">
            ...
            <div className="hidden sm:block">
              ...
              <span className="block text-[8px] text-[var(--amber)] font-bold tracking-widest">BUILDING MATERIALS</span>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-0.5">
            ...
            <Link href="/cart" className="relative flex flex-col items-center text-white/60 hover:text-white px-2.5 py-1.5 rounded-lg hover:bg-white/5 transition-all">
              ...
              <span className="absolute -top-0 right-1 h-[16px] min-w-[16px] rounded-full bg-[var(--amber)] text-[9px] font-black text-[var(--text)] flex items-center justify-center px-1">3</span>
            </Link>
          ...
          <div className="max-w-[1440px] mx-auto flex items-center overflow-x-auto px-4 sm:px-6 scrollbar-hide">
            {["Today's Deals", "Cement", "Steel & TMT", "Tiles", "Paint", "Electrical", "Plumbing", "Bulk Orders", "New Arrivals", "All Categories"].map(item => (
              <Link key={item} href={item === "Today's Deals" ? "/products?sort=deals" : item === "All Categories" ? "/products" : `/products?category=${item.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-')}`}
                className="shrink-0 px-3 py-2.5 text-[12px] font-medium text-white/70 hover:text-white hover:bg-white/5 transition-all border-b-2 border-transparent hover:border-[var(--amber)]">
                {item}
              </Link>
            ))}
          </div>
      ...
      <main className="max-w-[1440px] mx-auto px-4 sm:px-6">
        ...
        <section className="pt-4 pb-3">
          <div className="grid gap-3 lg:grid-cols-[1fr_260px]">
            ...
            <div className="relative overflow-hidden rounded-lg bg-white border border-[var(--border)] min-h-[340px]">
              <div className="relative h-full flex flex-col justify-center p-8 sm:p-10 lg:p-12">
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                  <Link href={s.link}
                    className={s.ctaStyle === "amazon" ? "btn-amber" : s.ctaStyle === "blue" ? "btn-brand" : "btn-teal"}>
                  ...
                  <motion.div key={`hero-${i}`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-baseline gap-1 mt-0.5">
                        <span className="text-[14px] font-black text-[var(--text)]">â‚¹{p.price.toLocaleString()}</span>
                        <span className="text-[10px] text-[var(--text-muted)] line-through">â‚¹{p.mrp.toLocaleString()}</span>
                      </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              {/* Download App */}
              <div className="rounded-lg bg-[var(--text)] p-3 text-white">
              ...
              <div className="card p-3">
                ...
                <AnimatePresence mode="wait">
                  <motion.div key={liveIdx} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}
                    ...
                    <p className="text-[10px] text-[var(--text-muted)] mt-0.5">{LIVE_ORDERS[liveIdx].city} â€” {LIVE_ORDERS[liveIdx].time}</p>
                  </motion.div>
                </AnimatePresence>
              </div>
            </div>
        ...
        <section className="py-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              { label: "Customer Rating", value: "4.8â˜…", icon: Star, color: "#7C3AED" },
            ].map((stat, i) => (
        ...
        {/* â”€â”€ CTA Banner â”€â”€ */}
        <section className="py-4">
          <div className="rounded-lg bg-gradient-to-r from-[var(--text)] to-[#1E293B] p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
            ...
            <div className="flex gap-2">
              <Link href="/products" className="btn-amber">Start Ordering</Link>
      ...
      <footer className="bg-[var(--text)] mt-6">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="border-t border-white/5 mt-6 pt-5 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-[10px] text-white/30">2026 MODIT. All rights reserved. Made with â¤ï¸ in India.</p>
